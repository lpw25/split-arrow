...
module Simple = struct
  module Make (Data : Data.S) = struct
    module Key = struct
      ...
      module type S = sig
        type index
        type 'a repr += C : index repr
      end

      type 'a t = (module S with type index = 'a)
      ...
      let id (type a) ((module M) : a t) = [%extension_constructor M.C]

      module Packed = struct
        type 'a unpacked = 'a t
        type t = Packed : 'a unpacked -> t [@@unboxed]
    ...
    module Sigma = struct
      type 'a t = Sigma : { key : 'b Key.t; data : ('a, 'b) Data.t } -> 'a t
    end

    type 'a t = 'a Sigma.t Map.t
    ...
    let find (type a b) (t : a t) (key : b Key.t) : (a, b) Data.t option =
      let packed = Key.Packed.Packed key in
...
module Classified = struct
  module Make (Data : Data.S) = struct
    module Key = struct
      ...
      let equal (type a b k j) ((module A) : (a, k) t) ((module B) : (b, j) t) :
          (a, b) Eq.t option =
        match A.C with B.C -> Some Eq.Refl | _ -> None
